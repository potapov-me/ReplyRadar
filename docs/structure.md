...
**Внутрипроектные импорты — абсолютные.** Модули внутри `src/replyradar/` импортируются от корня пакета `replyradar`, без `from ..` и `from .`. Подробнее: ADR-0018.
...
**Репозитории остаются основным местом для SQL.** Повторно используемые read/write-запросы и SQL с доменной семантикой должны жить в `db/repos/`. При этом в текущем коде есть точечные прямые обращения к `pool` и локальный SQL вне `db/repos/` для orchestration и status/read-model путей; документация не рассматривает это как отдельный архитектурный слой и не утверждает, что таких вызовов нет. Рекурсивные CTE, pgvector-операторы и `ON CONFLICT` пишутся как явный SQL — без ORM-обёрток.
