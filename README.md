...
Этап 2 завершён: ingestion работает.

| Этап | Статус | Что |
|------|--------|-----|
| 1. Фундамент | ✓ | src-layout, config, DB pool, Alembic схема, `GET /status`, import boundaries |
| 2. Ingestion | ✓ | Telethon listener, backfill батчами, `POST /chats/{id}/monitor`, `POST /backfill` |
...
- [Как работает ingestion](./docs/ingestion.md)
